...
{
	fn new<E: Executable<T>>(contract: &AliveContractInfo<T>, executable: &E) -> Self {
		Self {
	...
	E: Executable<T>,
{
	...
	fn new_call_context<'b>(
		...
		CallContext {
			...
			timestamp,
			block_number,
			rent_params: RentParams::new(contract, executable),
			_phantom: Default::default(),
		}
	}
...
mod tests {
	...
	fn rent_params_works() {
		let code_hash = MockLoader::insert(Call, |ctx, executable| {
			let contract = <ContractInfoOf<Test>>::get(ctx.ext.address())
				.and_then(|c| c.get_alive())
				.unwrap();
			assert_eq!(ctx.ext.rent_params(), &RentParams::new(&contract, executable));
			exec_success()
		});
	...
	fn rent_params_snapshotted() {
		let code_hash = MockLoader::insert(Call, |ctx, executable| {
			...
			let contract = <ContractInfoOf<Test>>::get(ctx.ext.address())
				.and_then(|c| c.get_alive())
				.unwrap();
			let rent_params = RentParams::new(&contract, executable);
